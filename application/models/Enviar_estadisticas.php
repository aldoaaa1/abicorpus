	...
	class Enviar_estadisticas extends CI_Model {
		...
		function registrar_visita(){
			...
			$data = array(
				...
				// Variables añadidas gracias a la API de https://api.db-ip.com/v2/free/self
				'api_ip' => $_POST['user']['api_ip'],
				'api_continent' => $_POST['user']['api_continent'],
				'api_country' => $_POST['user']['api_country'],
				'api_state' => $_POST['user']['api_state'],
				'api_city' => $_POST['user']['api_city']
			);
		...
		function registrar_frecuencia(){
			...
			$query = 'SELECT idvisitas_frecuencia, cantidad_visitas FROM frecuencia where key_visita = \''.$identificador.'\'';
			...
			if ($es_nuevo) {
				...
				$this->db->insert('frecuencia', $data);
			} else {
				$cantidad_visitas = 
