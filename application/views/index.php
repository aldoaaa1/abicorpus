<div class="top">
	<div>
		<div class="t-left">
			<a href="<?=base_url()?>book_now">Agendar tu Cita</a>
			<p>Tel: 81292390</p>
		</div>
		<div class="t-right sm-hide">
			<a href="https://www.facebook.com/abicorpuss" target="_blank">Facebook</a>
			<a href="https://www.instagram.com/abicorpushairstudio" target="_blank">Instagram</a>
			<a href="https://api.whatsapp.com/send?phone=528110041116&text=%C2%A1Hola!%20Encontr%C3%A9%20su%20contacto%20en%20su%20P%C3%A1gina%20Web." target="_blank">Wp: 8110041116</a>
		</div>
	</div>
</div>
<div id="header1" class="container">
	<menu>
		<img src="<?=base_url()?>img/AbiNegro.svg" alt="">
		<div class="line1"></div>
		<div class="links onScroll" data-type="show" data-delay="800">
			<a href="<?=base_url()?>">Home</a>
			<a href="<?=base_url()?>services">Services</a>
			<a href="<?=base_url()?>book_now">Book Now</a>
			<a href="<?=base_url()?>contact">Contact</a>
		</div>
		<div id="phone_menu" onclick="openPhoneMenu();">
			<div class="line"></div>
			<div class="line"></div>
			<div class="line"></div>
		</div>
	</menu>
	
</div>
<div id="header2" class="container">
	<h1 class="onScroll" data-type="show" data-delay="500">Abi Corpus</h1>
	<img class="onScroll" data-type="up" data-delay="650" src="img/header2.jpeg" alt="Salón de Belleza">
	<div class="h_footer">
		<div class="onScroll" data-type="left">
			<h3>Dirección</h3>
			<p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Id, quas.</p>
		</div>
		<div class="separador"></div>
		<div class="onScroll" data-type="right">
			<h3>Contacto</h3>
			<p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Id, quas.</p>
		</div>
	</div>
</div>
<div class="onScroll" data-type="up" data-delay="200" id="quienes_somos" class="full_container">
	<img src="img/header2.jpeg" alt="Abi Corpus Salón de Belleza">
	<div class="container">
		<img src="img/abi1.jpeg" alt="">
		<div class="q1">
			<h2>¿Quiénes Somos?</h3>
			<p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Impedit blanditiis, numquam. Quos nihil, numquam voluptate delectus voluptates, labore. Cupiditate, impedit?</p>
			<p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Neque, sint, officiis. Qui hic reprehenderit iste, aliquam necessitatibus aperiam, vitae enim aspernatur minus, magni labore odit error maxime sit, animi nostrum.</p>
			<a href="<?=base_url()?>contact">Contacto</a>
		</div>
	</div>
</div>
<div id="servicios" class="container">
	<h2 class="onScroll" data-type="show" data-delay="200">Servicios</h3>
	<div class="serv">
		<div class="servicio onScroll" data-type="up">
			<img src="img/corte.jpg" alt="Servicio de Belleza">
			<h6>Corte</h6>
		</div>
		<div class="servicio onScroll" data-type="up" data-delay="200">
			<img src="img/despunte.png" alt="Servicio de Belleza">
			<h6>Despunte</h6>
		</div>
		<div class="servicio onScroll" data-type="up" data-delay="400">
			<img src="img/laminado.jpg" alt="Servicio de Belleza">
			<h6>Laminado de Ceja</h6>
		</div>
		<div class="servicio onScroll" data-type="up" data-delay="600">
			<img src="img/peinado_maquillaje.jpg" alt="Servicio de Belleza">
			<h6>Peinado y Maquillaje</h6>
		</div>
		<div class="servicio onScroll" data-type="up" data-delay="800">
			<img src="img/uñas_acrilicas.jpeg" alt="Servicio de Belleza">
			<h6>Uñas de Acrilico</h6>
		</div>
	</div>
	<a class="onScroll" data-type="up" href="<?=base_url()?>services">Ver Precios</a>
</div>
<div id="ubicacion" class="full_container onScroll" data-type="up" data-delay="200">
	<h2>Ubicación</h2>
	<iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3596.349482141825!2d-100.2575392846131!3d25.659707219076267!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x8662c13786b1e12b%3A0x6bc82b992c2ded8f!2sAbi%20Corpus%20Hair%20Studio!5e0!3m2!1ses!2smx!4v1645118597639!5m2!1ses!2smx" style="border:0;" allowfullscreen="" loading="lazy"></iframe>
</div>
<?php include('template/footer1.php') ?>


<link rel="stylesheet" href="https://creacionweb.com.mx/code/chatbox.css">
<div id="chatbox">
	<div class="chatbox-asesor" data-img="<?=base_url()?>img/user.jpg" data-nombre="User" data-puesto="Técnica de Color" data-correo="aldo.aaa1@gmail.com"></div>
	<div class="chatbox-question" data-question="¡Hola! ¿Que servicio te interesa?"></div>
	<div class="chatbox-question" data-question="¿En que correo o teléfono podría contactarte?"></div>
	<div class="chatbox-question" data-question="¿Con quién tengo el gusto de conversar?"></div>
	<div class="chatbox-customColors" data-body="#f0efef" data-chatboxmsg="#FFF0F0" data-clientmsg="#fce8f9" data-iconhover="#FFF0F0"></div>
	<div class="chatbox-comportamiento" data-delay_autoshow_ms="18000" data-close_message="¡Muchas gracias por tu tiempo! Me contactaré contigo personalemte lo mas pronto posible. También estoy para servirte en el correo: aldo.aaa1@gmail.com"></div>
</div>
<script src="<?=base_url()?>js/chatbox.js"></script>